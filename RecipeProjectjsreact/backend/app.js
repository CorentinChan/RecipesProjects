...
// error of multer ( size limit , type....)
...
app.post('/setRecipeID', async (req, res) => {
	...
	else if (req.cookies.recipeID) {
		recipeID = req.cookies.recipeID; //req.session.recipeID = recipeID;   
	}
	else {
				...
				var query = connection.query('UPDATE recipe SET ? where id= ?', [post, recipeModID],
					function (error, results, fields) {
						...
						if (req.body.tags && req.body.tags != 0) {
							tag = req.body.tags.filter(item => item !== null && item !== "");
							console.log(tag);
						}
						...
						if (req.body.tags && req.body.tags != "") {
							const tagPost = tag.map(txt => [txt, recipeModID]);
...
const storage = multer.diskStorage({
	...
	filename: (req, file, cb) => {
		// never keep origine name  for security
		// generate UUID + keep extension
...
// files filter type and
const fileFilter = (req, file, cb) => {
	// SÉCURITÉ : N'accepter que certains types MIME
	const allowedTypes = ['image/jpeg', 'image/png', 'application/pdf'];

	if (allowedTypes.includes(file.mimetype)) {
		cb(null, true);
	} else {
		cb(new Error('invalid file type. Only jpe png are authorized'), false);
	}
};
...
app.post('/createRecipe', upload.single('image'), (req, res) => { //create recipe save on database
	...
	if (req.cookies.pseudoID) { //create post for recipe table
		...
		let post = {
			...
			image: imageFilename, // On stocke le nom du fichier généré sur le serveur
			totalTime: recipeData.totalTime,
			yield: recipeData.nbPerson,
			activeTime: recipeData.activeTime,
			auteur: recipeData.author,
		...
		var query = connection.query('INSERT INTO recipe SET ?', post,
			function (error, results, fields) {
				...
				// prepapre steps tags ingredients data
				let stepsRaw = recipeData.steps || [];
				let step = stepsRaw.filter(item => item !== null && item !== "");

				let tagsRaw = recipeData.tags || [];
				let tag = tagsRaw.filter(item => item !== null && item !== "");

				console.log(tag);

				let ingredient = recipeData.ingredients;
				let measure = recipeData.measures;

				// filter with same table to delete only empty ingredient name
				let filtered = ingredient
...
app.post('/deleteRecipe', async (req, res) => {
	...
	try {
		let recipeID = req.body.recipeID;
		console.log("la recette a supprimee :" + recipeID);
		const [recipe] = await pool.execute(`SELECT * FROM recipe WHERE id = ? `, [recipeID]);
		console.log(recipe);
		let url = recipe[0].image;
		// regex to get name files
		const regex = /uploads\/(.+)$/;
		const match = url.match(regex);
		let filename = '';

		if (req.cookies.userRole != "admin" || req.cookies.userID) res.json({ checkUser: false });
		else {
			if (match) {
				...
				if (fs.existsSync(filePath)) {
					fs.unlink(filePath, (err) => {
						if (err) { console.error("error during suppresion :", err); }
						else { console.log("file deleted :", filename); }
					});
				} else { console.log(" file not found :", filename); }
			}

			const [deleteRecipe] = await pool.execute(
				`DELETE FROM recipe WHERE id = ? `
				, [recipeID]
			);
			await pool.execute(`DELETE FROM instructions WHERE recipeID = ?`, [recipeID]);
			await pool.execute(`DELETE FROM liste_ingredients WHERE recipeID = ?`, [recipeID]);
			await pool.execute(`DELETE FROM tagslist WHERE recipeID = ?`, [recipeID]);
			await pool.execute(`DELETE FROM commentaires WHERE recipeID = ?`, [recipeID]);
			await pool.execute(`DELETE FROM notes WHERE recipeID = ?`, [recipeID]);
			await pool.execute(`DELETE FROM recipe WHERE id = ?`, [recipeID]);
	...
	connection.query(`SELECT recipeID FROM tagslist WHERE tag = ? ;SELECT id FROM category WHERE name = ? ; SELECT recipeID FROM liste_ingredients WHERE ingredient = ? ; `,
		[textSearch, textSearch, textSearch], function (error, result, fields) {
			...
			if (req.cookies.pseudoID) {
				recipeList = `LEFT JOIN recipe_list on type="favoris"
