...
async function getID(cat) {
    ...
    try {
        ...
        // Note: Idéalement il faudrait utiliser util.promisify pour connection.query, mais on fait simple ici
        ...
        if (categoryID) {
            ...
            for (const element of recipes) {
                ...
                // PAUSE IMPORTANTE : 200ms entre chaque recette pour ne pas crash le serveur
                await wait(200); 
            }
        }
...
async function copyMealByID(mealID, categoryID) {
    ...
    try {
        // CORRECTION : Ajout de axiosConfig ici aussi
        const response = await axios.get(url, axiosConfig);
