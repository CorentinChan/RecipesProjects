  ...
  <div className="p-4 border rounded-4 shadow-sm text-white bg-danger">
    ...
    <form className=" contactForm " onSubmit={handleSubmit}>
      <div className="  w-100 w-lg-75 mx-auto">
        ...
        <input type="text" className="form-control rounded-pill bg-danger border border-white text-white" 
        id="name" onChange={(e)=>(setName(e.target.value))} value={name} minLength={3} />
        
        <label htmlFor="mail" className="form-label fs-4 me-3 mt-3">Email</label>
        <input type="email" className="form-control rounded-pill bg-danger border border-white text-white"
         id="mail" onChange={(e)=>(setEmail(e.target.value))} value={email} />
      
        <label htmlFor="message" className="form-label fs-4 me-3 mt-3 ">message</label>
        <input type="text" className="form-control rounded-4 bg-danger border border-white text-white  messageForm"
         id="message" onChange={(e)=>(setMessage(e.target.value)) } value={message} minLength={2}/>
  
   
      </div>
      <div className=" text-center">
      {  (response!='Mail sent!')?<button type="submit" 
       className="btn mx-3 fs-5 px-3 mt-3 btn-warning  rounded-pill">Send message</button>
       :undefined}
    </div>
